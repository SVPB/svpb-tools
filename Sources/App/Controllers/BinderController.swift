...
struct BinderController: RouteCollection {
    ...
    func binderConstructorPage(req: Request) async throws -> View {
        try await req.view.render("binder-constructor", [
    ...
    func binderBuilderPage(req: Request) async throws -> View {
        var context: [String: String] = ["appVersion": AppVersion.current]
        if let encodedSpec = req.query[String.self, at: "spec"] {
            context["encodedSpec"] = encodedSpec
        }
        return try await req.view.render("binder-builder", context)
    }
    ...
    func createBinder(req: Request) async throws -> Response {
        ...
        Task {
            await binderService.generateBinder(requestID: id, db: db, logger: logger)
        }

        let dto = try BinderStatusDTO(from: binderRequest, baseURL: baseURL(for: req))
    ...
    func binderStatus(req: Request) async throws -> BinderStatusDTO {
        ...
        return try BinderStatusDTO(from: binderRequest, baseURL: baseURL(for: req))
    }
    ...
    private func baseURL(for req: Request) -> String {
        let domain = Environment.get("DOMAIN") ?? "localhost:8080"
        let scheme = domain.hasPrefix("localhost") ? "http" : "https"
